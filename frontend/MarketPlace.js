  ...
  for (const docSnap of reqSnap.docs) {
    ...
    if (req.status === "open" && req.userUid !== supplierUid) {
      ...
      (async () => {
        ...
        if (seasonal?.demand) {
          const titleExtra = seasonal?.increaseRange ? `\nSuggested increase: ${seasonal.increaseRange}` : '';
          const seasonalTagHtml = `<span class="seasonal-tag" title="${(seasonal.reason || '') + titleExtra}">${seasonal.recommendation}</span>`;
      ...
      (async () => {
        ...
        if (market?.demand) {
          const titleExtra = market?.increaseRange ? `\nSuggested increase: ${market.increaseRange}` : '';
          const recommendTagHtml = `<span class="rec-tag" title="${(market.reason || '') + titleExtra}">Recommended</span>`;
          const insertedCard = marketplaceList.querySelector(`.deal-card[data-req-id="${docSnap.id}"]`);
          if (insertedCard) {
